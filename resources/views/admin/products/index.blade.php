                ...
                <a href="{{ route('admin.products.import') }}" class="btn btn-secondary shadow-md shadow-secondary/20">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24"
                        ...
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M4 4v16h16V4H4zm4 4h8v2H8V8zm0 4h8v2H8v-2zm0 4h8v2H8v-2z" />
                    </svg>
                    Bulk Import
                </a>
            ...
            <div class="flex items-center gap-2">
                ...
                @if (request('filter'))
                    <div class="badge badge-primary badge-sm font-bold uppercase">
                        {{ $products->total() }} {{ str_replace('_', ' ', request('filter')) }}</div>
                @endif
            </div>

            <div class="join border border-base-300 shadow-sm">
                <a href="{{ route('admin.products.index') }}"
                    class="join-item btn btn-xs px-4 {{ !request('filter') ? 'btn-active bg-base-300' : 'bg-base-100' }}">
                    All
                </a>

                <a href="{{ route('admin.products.index', ['filter' => 'low_stock']) }}"
                    class="join-item btn btn-xs px-4 {{ request('filter') == 'low_stock' ? 'btn-active btn-warning' : 'bg-base-100' }}">
                    Low Stock
                </a>

                <a href="{{ route('admin.products.index', ['filter' => 'out_of_stock']) }}"
                    class="join-item btn btn-xs px-4 {{ request('filter') == 'out_of_stock' ? 'btn-active btn-error text-white' : 'bg-base-100' }}">
                    Out of Stock
                </a>
            </div>
        </div>
                    ...
                    @if (!$products->isEmpty())
                        @foreach ($products as $product)
                            <tr class="hover:bg-base-200/30 transition-colors">
                                <td>
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-[10px] opacity-50 font-mono italic">
                                                #ID-{{ $product->id }}
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <td class="text-center">
                                    @if ($product->stock <= 0)
                                        <div class="badge badge-error gap-1 text-[10px] font-bold uppercase">Out of
                                            Stock
                                        </div>
                                    @elseif($product->stock <= 5)
                                        <div class="badge badge-warning gap-1 text-[10px] font-bold uppercase">Low stock:
                                            {{ $product->stock }}</div>
                                    @else
                                        <div
                                        ...
                                        <dialog id="delete_modal_{{ $product->id }}"
                                            ...
                                            <div class="modal-box border border-error/20">
                                                <div class="flex items-center gap-4 text-error mb-4">
                                                    ...
                                                    <h3 class="font-black text-lg text-base-content">Are you sure you
                                                        want
                                                        to delete?
                                                    </h3>
                                                </div>
                            ...
                            <tr>
                                <td colspan="5" class="text-center py-20">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-lg font-bold opacity-50">No
                                            {{ str_replace('_', ' ', request('filter')) }} products found</p>
                                    </div>
                                </td>
                            </tr>
                        @endif
